...
function Body(){
  const ctx = useContext(context)
  const {error, weather, mode, searched} = ctx
  let data
  if(weather){
    console.log(weather)
    data = {
      labels: weather[mode].data.map(d=> {
        let format = 'ddd'
        if(mode==='hourly') format='ddd hh:mm'
        return moment(d.time*1000).format(format)
      }),
      datasets: [{
        label: (searched + ' Temperature'),
...
async function search({searchTerm, searched, set}){
  try {
    ...
    set({weather, loading:false, searchTerm:'', searched: cap(term)})
  } catch(e) {
    set({error: e.message})
  }
}
